...
def filter_df(df, symbol = "BTCUSDT", date_range = None):
    ...
    if "open_datetime" in filtered_df.columns:
        filtered_df["open_datetime"] = pd.to_datetime(filtered_df["open_datetime"], utc=True)
    if "close_datetime" in filtered_df.columns:
        filtered_df["close_datetime"] = pd.to_datetime(filtered_df["close_datetime"], utc=True)
    ...
    if date_range and len(date_range) == 2:
        ...
        mask = (filtered_df[time_col] >= start_date) & (filtered_df[time_col] <= end_date)
