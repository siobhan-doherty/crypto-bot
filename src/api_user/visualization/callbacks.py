...
# module level globals, set inside register_callbacks
_full_df: pd.DataFrame = pd.DataFrame()
_realtime_store: Dict[str, pd.DataFrame] = {}
...
def update_real_time(
    ...
    _realtime_store[symbol] = df
    if not trading_pair:
        return dash.no_update, status

    selected_df = _realtime_store.get(trading_pair)
...
def update_volatility(atr_period: int, date_range: Any, _: Any) -> go.Figure:
    pair_data = {}
    for pair in VOLATILITY_TRADING_PAIRS:
        filtered = _filtered_dataframe_or_empty(_full_df, pair, date_range)
