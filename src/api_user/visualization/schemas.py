...
class KlineData(BaseModel):
    ...
    def validate_prices(self) -> "KlineData":
        ...
        if self.high < self.low:
            raise ValueError(f"High price {self.high} is less than low price {self.low}")
        if self.close > self.high:
            raise ValueError(f"Close price {self.close} is greater than high price {self.high}")
        if self.close < self.low:
            raise ValueError(f"Close price {self.close} is less than low price {self.low}")
        return self
...
class StreamingData(BaseModel):
    ...
    close: float
    ts: datetime
    is_closed: bool = Field(..., description = "WebSocket flag indicating if the kline is closed")
