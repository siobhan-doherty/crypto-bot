...
This script is used for development purposes only and is not intended for production.
...
def wait_for_api():
    ...
    while attempt < max_attempts:
        ...
        if attempt % 5 == 0:
            print(
                f"   Waiting for FastAPI server... (attempt {attempt}/{max_attempts})"
            )
...
def main():
    ...
    if command == "api":
        ...
        stdout_thread = threading.Thread(
            target=print_output, args=(api_process.stdout, "[API]")
        )
        stderr_thread = threading.Thread(
            target=print_output, args=(api_process.stderr, "[API ERROR]")
        )
